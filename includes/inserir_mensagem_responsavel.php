...
if (isset($_POST['mandar'])) {
    ...
    $remetente = $_SESSION['cd_usuario']; 
      ...
      if ($query_tipo_p->num_rows>0) {
        ...
        $sql_recado_p = 'insert into tb_mensagem_responsavel values(null, "'.$recado.'", '.$_SESSION['cd_usuario'].', '.$row_p['cd_professor'].', null, null, "'.date('d/m/y').'", "'.date('H:i:s').'")';
      ...
      if ($query_tipo_a->num_rows>0) {
         ...
         $sql_recado_a = 'insert into tb_mensagem_responsavel values(null, "'.$recado.'", '.$_SESSION['cd_usuario'].', null, null, '.$row_a['cd_administrador'].', "'.date('d/m/y').'", "'.date('H:i:s').'")';
         echo $sql_recado_a;
      ...
      if ($query_tipo_r->num_rows>0) {
       $row_r = $query_tipo_r->fetch_array();
        ...
        $sql_recado_r = 'insert into tb_mensagem_responsavel values(null, "'.$recado.'", '.$_SESSION['cd_usuario'].', null, '.$row_r['cd_responsavel'].', null, "'.date('d/m/y').'", "'.date('H:i:s').'")';
        echo $sql_recado_r;
